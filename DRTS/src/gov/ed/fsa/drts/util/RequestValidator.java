...
public class RequestValidator implements PhaseListener {
	...
	public void beforePhase(PhaseEvent phase_event) 
	{
		...
		final String incoming_user = request_header_map.get(ApplicationProperties.USER_HEADER.getStringValue());
		
		UserSession temp_user_session = (UserSession) faces_context.getExternalContext().getSessionMap().get(ApplicationProperties.USER_SESSION_HEADER.getStringValue());
			...
			if(Utils.isStringEmpty(incoming_user) == false)
			{
				...
				if(temp_user_session == null)
				{
					...
					if(identity_service != null)
					{
						...
						{
							List<Group> security_groups = identity_service.createGroupQuery().groupType(ApplicationProperties.GROUP_TYPE_SECURITY.getStringValue()).groupMember(activiti_user.getId()).list();
							...
							else
							{
								...
								destination_url = ApplicationProperties.CONTEXT_ROOT.getStringValue() + ApplicationProperties.PAGE_ERROR_UNAUTHORIZED.getStringValue();
							}
						}
						else
						{
					...
					else
					{
						...
						destination_url = ApplicationProperties.CONTEXT_ROOT.getStringValue() + ApplicationProperties.PAGE_ERROR_APPLICATION.getStringValue();
					}
				}
				else
				{
